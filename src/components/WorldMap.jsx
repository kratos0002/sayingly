...
const WorldMap = () => {
  ...
  // Countries we have idioms for - using ISO-3 country codes
  ...
  const handleCountryClick = (geo) => {
    ...
    console.log('Clicked country:', geo.properties);
    
    const countryCode = geo.properties.ISO_A3 || geo.id;
    if (highlightedCountries[countryCode]) {
      if (Array.isArray(highlightedCountries[countryCode])) {
        navigate(`/country/${countryCode}`);
      } else {
        navigate(`/language/${highlightedCountries[countryCode]}`);
      }
    }
  };
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          ...
          >
            <Geographies geography="https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json">
              {({ geographies }) =>
                geographies.map((geo) => {
                  const isHighlighted = highlightedCountries[geo.properties.ISO_A3 || geo.id];
                  return (
                    <Geography
                      key={geo.rsmKey}
                      geography={geo}
                      ...
                      onMouseEnter={() => {
                        console.log('Hovering:', geo.properties.name);
                      }}
                      style={{
                        default: {
